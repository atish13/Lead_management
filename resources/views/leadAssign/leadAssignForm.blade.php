...
<x-guest-layout>
    <form method="POST" action="{{ route('leadassign.store') }}">
        @csrf

        <!-- Name -->
        <div>
            <x-input-label for="lead id" :value="__('lead id')" />
            <x-text-input id="lead id" class="block mt-1 w-full" type="text" name="lead_id" :value="old('lead id')" required autofocus autocomplete="name" />
